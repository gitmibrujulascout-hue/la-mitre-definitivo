...
export default function RendicionesList({ anio, totalExigidoSA: totalExigidoSAProp }) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Rendición eliminada y egreso revertido');
    }
  });

  const handleDelete = (r) => {
    if (window.confirm(`¿Eliminar el depósito del ${r.fecha} por ${formatMoney(r.monto_depositado)}? Se revertirá el egreso de caja.`)) {
  ...
  const { rendicionesCalculadas, resumen } = useMemo(() => {
    ...
    const totalDepositado = rendAnio.reduce((s, r) => s + (r.monto_depositado || 0), 0);
    const saldoADepositar = Math.max(0, totalExigidoSA - totalDepositado);

    return {
      rendicionesCalculadas: rendAnio,
      resumen: { totalExigidoSA, totalRecaudadoFamilias, totalDepositado, saldoADepositar },
    };
  ...
  return (
    <Card className="overflow-hidden mt-6">
      ...
      {/* Resumen financiero */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 bg-muted/10 border-b">
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50">
            ...
            <TableHead className="text-right">Depositado a SA</TableHead>
