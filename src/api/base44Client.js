...
const entity = name => ({
  ...
  async create(values) { const payload = entityValues(name, values); const tenant = await getActiveTenantId(); if (tenant && !payload.tenant_id) payload.tenant_id = tenant; const { data, error } = await supabase.from(snake(name)).insert(payload).select().single(); if (error) throw error; return data; },
  async update(id, values) { let q = supabase.from(snake(name)).update({ ...entityValues(name, values), updated_at: new Date().toISOString() }).eq('id', id); const tenant = await getActiveTenantId(); if (!tenant) throw new Error('No hay una organización activa.'); q = q.eq('tenant_id', tenant); const { data, error } = await q.select().single(); if (error) throw error; return data; },
  ...
  async bulkCreate(records) { const tenant = await getActiveTenantId(); const payload = records.map(row => entityValues(name, row)).map(row => tenant && !row.tenant_id ? { ...row, tenant_id: tenant } : row); const { data, error } = await supabase.from(snake(name)).insert(payload).select(); if (error) throw error; return data || []; },
  async bulkUpdate(filters, values) { let q = supabase.from(snake(name)).update({ ...entityValues(name, values), updated_at: new Date().toISOString() }); const tenant = await getActiveTenantId(); if (tenant) q = q.eq('tenant_id', tenant); for (const [key, value] of Object.entries(normalize(filters))) q = q.eq(key, value); const { data, error } = await q.select(); if (error) throw error; return data || []; }
});
