...
const entity = name => ({
  async list(sort = '-created_date', limit = name === 'Beneficiario' ? 1000 : 100) { const desc = String(sort).startsWith('-'); const col = snake(String(sort).replace(/^-/, '').replace(/_date$/, '_at')); let q = supabase.from(snake(name)).select('*'); const tenant = await getActiveTenantId(); if (tenant) q = q.eq('tenant_id', tenant); q = q.order(col, { ascending: !desc }); if (limit) q = q.limit(limit); const { data, error } = await q; if (error) throw error; return data || []; },
  async filter(filters = {}, sort = '-created_date', limit = name === 'Beneficiario' ? 1000 : 100) { const desc = String(sort).startsWith('-'); const col = snake(String(sort).replace(/^-/, '').replace(/_date$/, '_at')); let q = supabase.from(snake(name)).select('*'); const tenant = await getActiveTenantId(); if (tenant) q = q.eq('tenant_id', tenant); for (const [key, value] of Object.entries(normalize(filters))) q = value === null ? q.is(key, null) : q.eq(key, value); q = q.order(col, { ascending: !desc }); if (limit) q = q.limit(limit); const { data, error } = await q; if (error) throw error; return data || []; },
  async create(values) { const payload = normalize(values); const tenant = await getActiveTenantId(); if (tenant && !payload.tenant_id) payload.tenant_id = tenant; const { data, error } = await supabase.from(snake(name)).insert(payload).select().single(); if (error) throw error; return data; },
  async update(id, values) { let q = supabase.from(snake(name)).update({ ...normalize(values), updated_at: new Date().toISOString() }).eq('id', id); const tenant = await getActiveTenantId(); if (!tenant) throw new Error('No hay una organización activa.'); q = q.eq('tenant_id', tenant); const { data, error } = await q.select().single(); if (error) throw error; return data; },
  ...
  async deleteMany(filters = {}) { let q = supabase.from(snake(name)).delete(); const tenant = await getActiveTenantId(); if (tenant) q = q.eq('tenant_id', tenant); for (const [key, value] of Object.entries(normalize(filters))) q = q.eq(key, value); const { error } = await q; if (error) throw error; return true; },
  async bulkCreate(records) { const tenant = await getActiveTenantId(); const payload = normalize(records).map(row => tenant && !row.tenant_id ? { ...row, tenant_id: tenant } : row); const { data, error } = await supabase.from(snake(name)).insert(payload).select(); if (error) throw error; return data || []; },
  async bulkUpdate(filters, values) { let q = supabase.from(snake(name)).update({ ...normalize(values), updated_at: new Date().toISOString() }); const tenant = await getActiveTenantId(); if (tenant) q = q.eq('tenant_id', tenant); for (const [key, value] of Object.entries(normalize(filters))) q = q.eq(key, value); const { data, error } = await q.select(); if (error) throw error; return data || []; }
});
